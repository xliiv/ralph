...
from ralph.lib.mixins.models import get_field_by_relation_path
...
class AutocompleteWidget(forms.TextInput):
    ...
    def render(self, name, value, attrs=None):
        ...
        if self.admin_site._registry[rel_to].has_change_permission(
            self.request
        ):
            context['can_change_related'] = True
            context['change_related_template_url'] = self.get_related_url(
                info, 'change', value,
            )
        if self.admin_site._registry[rel_to].has_add_permission(
            self.request
        ):
            context['can_add_related'] = True
